  ...
  return new Promise((resolve, reject) => {
    ...
    this.setDocument = () => {
        ...
        this.lf[collectionName]
          ...
          .then(() => {
            ...
            if (docsToSet.length > 1) {
              logger.warn.call(
                this,
                `O'rnatish uchun ${JSON.stringify(docSelectionCriteria)} bilan (${docsToSet.length}) ta document topildi.`
              )
            }
          })
          .then(() => {
            docsToSet.forEach((docToSet, index) => {
              this.lf[collectionName]
                .setItem(docToSet.key, docToSet.newDocument)
                .then(_value => {
      ...
      this.setDocumentByKey = () => {
        this.lf[collectionName]
          .setItem(docSelectionCriteria, newDocument)
          .then(_value => {
            resolve(
              success.call(
                this,
    ...
    if (!newDocument) {
      this.userErrors.push(
        'set() metodi uchun yangi document objecti taqdim etilmagan. Objectdan foydalaning. Masalan: {id: 1, ism: "Otabek", yoshi: 19}.'
      )
    } else if (currentSelectionLevel === 'doc') {
      if (!(typeof newDocument == 'object' && newDocument instanceof Array == false)) {
        this.userErrors.push(
          "set() metodiga uzatilgan ma'lumotlar object bo'lishi kerak. Array, string, number yoki boolean emas."
        )
      }
    } else if (currentSelectionLevel === 'collection') {
      if (!(typeof newDocument == 'object' && newDocument instanceof Array == true)) {
        this.userErrors.push(
          ".set() ga uzatilgan ma'lumotlar object(lar)ning massiv(lari) bo'lishi kerak. Object, string, number yoki boolean emas."
        )
      }
    }
